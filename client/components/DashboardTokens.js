  ...
  render() {
    ...
    return (
      <DashboardSection
        ...
        rightButtonText={!apiTokensLimitReached ? strings.API_TOKEN_GENERATE_BUTTON : undefined}
        onRightButtonClick={!apiTokensLimitReached ? this._toggleModal : undefined}
      ...
      >
        ...
        {!this.props.loading &&
          userApiTokens.map((apiToken, index) => {
            const disableDelete = this.state.generatingApiToken || this.state.deletingApiToken;
            const visible = this.state.visibleTokens.indexOf(apiToken.value) !== -1;
            return (
              <div className={'Tokens-content'} key={`api-token-${index}`}>
                <div className={'Tokens-content-left'}>
                  <div className={'Tokens-content-cell-name'}>{apiToken.name}</div>
                  <div className={'Tokens-content-cell-token'}>
                    {visible ? (
                      <div className={'Tokens-content-token-visible'}>{apiToken.value}</div>
                    ) : (
                      <div
                        style={{
                          backgroundImage: `url(/static/secret-blur.webp)`,
                        }}
                        className={'Tokens-content-token-hidden'}
                      >
                        <Button onClick={() => this._showToken(apiToken.value)} type={'info'}>
                ...
                <div
                  className={`Tokens-content-cell-delete ${disableDelete
                    ...
                    : ''}`}
                  onClick={() => !disableDelete && this._deleteToken(apiToken.value)}
                >
                  <img src={'/static/ic_delete_white_24px.svg'} alt={'Delete'} />
                </div>
              </div>
            );
          })}
